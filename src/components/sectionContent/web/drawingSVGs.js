...
{`.drawing-example-two {
    ...
    stroke-dashoffset: 225;
            ...
            <p>So what's going on? Using <code>stroke-dasharray</code> we can define the length of a dash along the stroke - and by <i>knowing</i> that the whole path is 375 pixels, we're saying that the whole length of the path is a single dash.</p>
            ...
            <div className='flex flex--centre-vertical'>
                ...
                <pre><code>
{`<svg xmlns="http://www.w3.org/2000/svg" width="180" height="180" viewBox="0 0 180 180" fill="none">
    <path className='drawing-example-three'
...
@keyframes SVGDrawingExample {
    ...
    100% { stroke-dashoffset: 0; }
            ...
            <p>All that's left to do is to plug the logic previously described into an animation - starting from offsetting the dash as the value of the dasharray, rendering nothing, to then setting the offset of the dash to 0 over a second. The animation syntax is important of course, but I wont go over that, you can read more about that <a href={'//developer.mozilla.org/en-US/docs/Web/CSS/animation'} target='_blank' rel='noopener noreferrer'>here</a>.</p>
        </div>
        </>
    );
}