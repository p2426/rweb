...
export default function App() {
    return (
        <Router>
            ...
            <p>Wrapping the markup of the SPA in a <code>{'<Router>'}</code> tag gives more fine-grained control over what components are rendered when. A <code>{'<Switch>'}</code> is used to render components exclusively, not inclusively, so you can think of it like your standard programatical switch statement. <code>{'<Route>'}</code> tags are simply what is hit when a subdirectory is added to the URL. You <i>could</i> just omit the <code>{'<Switch>'}</code> but then you would have to be more careful about the order of the <code>{'<Route>'}</code> tags inside - with a switch, it will break on the first match. You can also tack on the <code>exact</code> attribute on a <code>{'<Route>'}</code> which is handy because a route of <code>{'/contact'}</code> will also match <code>{'/con'}</code>, or even <code>{'/'}</code> for that matter.</p>
